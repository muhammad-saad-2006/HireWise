        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 1, background: 'var(--border)' }}>
            {[
              ...
              { val: 75, suffix: '%', label: 'Pass Threshold' },
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 1, background: 'var(--border)' }}>
            {[
              ...
              { num: '04', title: 'Instant Report', desc: 'Get a detailed PDF with score gauge, rule breakdown, and personalised improvement tips.', icon: '📊' },
            ].map((step, i) => (
              <div key={i} style={{ background: 'var(--bg-primary)', padding: '48px 32px', position: 'relative', overflow: 'hidden' }}>
      ...
      <section ref={featRef} style={{ position: 'relative', zIndex: 1, padding: '100px 24px', borderTop: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: 'var(--border)' }}>
            {[
              { role: 'Software Engineer', skills: ['Python', 'Java', 'SQL', 'Git', 'Docker', 'REST API'], threshold: '75%' },
              { role: 'Data Analyst', skills: ['SQL', 'Python', 'Pandas', 'Statistics', 'Excel', 'Tableau'], threshold: '75%' },
              { role: 'Web Developer', skills: ['HTML', 'CSS', 'JavaScript', 'React', 'Node.js', 'Git'], threshold: '75%' },
              { role: 'Cybersecurity Analyst', skills: ['Network Security', 'Linux', 'Pen Testing', 'OWASP', 'SIEM'], threshold: '75%' },
              { role: 'ML Engineer', skills: ['Python', 'scikit-learn', 'PyTorch', 'NumPy', 'Statistics', 'Git'], threshold: '75%' },
            ].map((r, i) => (
              <div key={i}
                style={{
              ...
              >
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {r.skills.map(s => (
                      <span key={s} style={{ fontFamily: 'JetBrains Mono', fontSize: 11, padding: '3px 8px', background: 'var(--accent-dim)', color: 'var(--accent)', borderRadius: 4 }}>{s}</span>
                    ))}
                  </div>
                </div>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '120px 24px', textAlign: 'center', borderTop: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginBottom: 40, fontSize: 16, fontWeight: 300 }}>
            Upload your CV. Get a detailed AI evaluation report instantly.
          </p>
