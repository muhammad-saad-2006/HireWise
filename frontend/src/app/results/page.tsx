...
function ScoreGauge({ score, passed }: { score: number, passed: boolean }) {
  ...
  const offset = animated ? circumference * (1 - score) : circumference
  const color = passed ? 'var(--success)' : score >= 0.65 ? 'var(--accent)' : 'var(--danger)'
  const label = passed ? 'INTERVIEW INVITED' : score >= 0.65 ? 'CLOSE — KEEP IMPROVING' : 'NEEDS WORK'
    ...
    <div ref={ref} style={{ textAlign: 'center', padding: '48px 32px' }}>
      <div style={{ position: 'relative', width: 240, height: 130, margin: '0 auto 16px' }}>
        <svg viewBox="0 0 220 120" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
          <path d="M 20 110 A 90 90 0 0 1 200 110" fill="none" stroke="var(--border)" strokeWidth="14" strokeLinecap="round" />
          <path d="M 20 110 A 90 90 0 0 1 200 110" fill="none" stroke={color} strokeWidth="14" strokeLinecap="round"
          ...
          {[0, 25, 50, 75, 100].map((t, i) => {
            ...
            return <line key={i} x1={110 + 90 * Math.cos(rad)} y1={110 + 90 * Math.sin(rad)}
              x2={110 + 78 * Math.cos(rad)} y2={110 + 78 * Math.sin(rad)} stroke="var(--text-muted)" strokeWidth="1.5" />
          })}
        </svg>
      ...
      <div style={{
        display: 'inline-block', padding: '8px 20px', borderRadius: 6, marginBottom: 8,
        background: passed ? 'var(--success-dim)' : score >= 0.65 ? 'var(--accent-dim)' : 'var(--danger-dim)',
        fontFamily: 'JetBrains Mono', fontSize: 12, fontWeight: 700, color, letterSpacing: '0.1em',
      }}>
        {label}
      </div>
  ...
  return (
    <div ref={ref} style={{
      ...
      <div style={{ flexShrink: 0, marginLeft: 16, textAlign: 'right' }}>
        <div style={{ fontFamily: 'JetBrains Mono', fontSize: 13, fontWeight: 700, color: rule.passed ? 'var(--success)' : 'var(--danger)' }}>
  ...
  return (
    <div ref={ref}>
      ...
      <div style={{ height: 4, background: 'var(--border)', borderRadius: 2 }}>
        <div style={{ height: '100%', background: 'var(--accent)', borderRadius: 2, width: inView ? `${(value / max) * 100}%` : '0%', transition: 'width 1.2s cubic-bezier(0.34, 1.56, 0.64, 1)' }} />
      </div>
    </div>
  )
}
...
function ResultsContent() {
  ...
  useEffect(() => {
    if (raw) { try { setData(JSON.parse(decodeURIComponent(raw))) } catch { setData(null) } }
  }, [raw])
  ...
  const allRules = [...passed.map((r: any) => ({ ...r, passed: true })), ...failed.map((r: any) => ({ ...r, passed: false }))]
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '48px 24px', position: 'relative', zIndex: 1 }}>
      ...
      <div style={{ marginBottom: 40 }}>
        ...
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ fontFamily: 'JetBrains Mono', fontSize: 12, color: 'var(--text-muted)' }}>{data.candidate_email}</span>
          <span style={{ fontFamily: 'JetBrains Mono', fontSize: 12, color: 'var(--accent)' }}>{data.role_title?.toUpperCase()}</span>
          {data.company_name && data.company_name !== 'HireWise' && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: 'var(--border)', marginBottom: 1 }}>
        <div className="card" style={{ borderRadius: 0, border: 'none' }}>
          <ScoreGauge score={data.score} passed={data.passed} />
        </div>
      ...
      {/* Rules detail */}
      <div className="card" style={{ marginBottom: 1, borderRadius: 0, border: 'none', borderTop: '1px solid var(--border)', padding: 32 }}>
      ...
      {/* Improvement tips */}
      {!data.passed && data.improvement_tips?.length > 0 && (
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 32, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {data.report_path ? (
          <a
            href={`http://localhost:8000${data.report_path}`}
            target="_blank"
            rel="noreferrer"
